...
func ListWishes(c *gin.Context) {
    ...
    if len(items) == 0 {
        ...
        type defWish struct{ name, content, unit string; coins, amount int }
        defaults := []defWish{
            {"看电视", "1金币可兑换看10分钟电视", "分钟", 1, 10},
            {"零花钱", "1金币可兑换1元零花钱", "元", 1, 1},
...
func UpdateWish(c *gin.Context) {
    ...
    if req.Content != nil { w.Content = *req.Content }
    if req.Icon != nil { w.Icon = *req.Icon }
    if req.NeedCoins != nil { w.NeedCoins = *req.NeedCoins }
    if req.ExchangeAmount != nil { w.ExchangeAmount = *req.ExchangeAmount }
    if req.Unit != nil { w.Unit = *req.Unit }
